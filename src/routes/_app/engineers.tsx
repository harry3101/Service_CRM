...
function Engineers() {
  ...
  const load = async () => { const { data } = await supabase.from("engineers").select("*").order("name"); setRows(data ?? []); };
  useEffect(() => { load(); }, []);

  const create = async (e: React.FormEvent) => {
    e.preventDefault();
    const { error } = await supabase.from("engineers").insert(form);
  ...
  const setStatus = async (id: string, status: string) => {
    await supabase.from("engineers").update({ status }).eq("id", id); load();
  };

...
function Dashboard() {
  ...
  useEffect(() => {
    (async () => {
      const [calls, rev, inv] = await Promise.all([
        supabase.from("service_calls").select("id,ticket_no,product,status,priority,created_at,customers(name,city)").order("created_at", { ascending: false }),
        supabase.from("revenue").select("amount,tax,invoice_date,status"),
        supabase.from("inventory").select("stock_qty,reorder_level"),
      ]);

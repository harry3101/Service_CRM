        ...
        <form onSubmit={save} className="space-y-3 text-xs">
          <div>
            ...
            <input
              value={user?.email ?? ""}
              disabled
              className="w-full rounded border border-input bg-secondary px-2 py-1.5"
            />
          </div>
          <div>
            <label className="mb-1 block text-[10.5px] font-medium uppercase text-muted-foreground">
          ...
          <button
            disabled={busy}
            className="rounded bg-primary px-3 py-1.5 text-primary-foreground hover:bg-primary/90 disabled:opacity-50"
          >
      ...
      <Panel title="Account & Permissions">
        <div className="space-y-2 text-xs">
              ...
              <div className="flex flex-wrap gap-1">
                {roles.length === 0 ? (
                  ...
                  roles.map((r) => (
                    <span
                      key={r}
                      className="rounded border border-primary/30 bg-primary/10 px-1.5 py-0.5 text-[10.5px] uppercase text-primary"
                    >
                      {r}
                    </span>
                  ))
          ...
          <p className="pt-2 text-[11px] text-muted-foreground">
            Roles control who can create, update or delete CRM records. Contact a workspace admin to
            change roles.
          </p>
        </div>
      </Panel>

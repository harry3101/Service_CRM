...
function Calls() {
  ...
  const load = async () => {
    const data = (await api.getServiceCalls()) as any[];
    setRows(data ?? []);
  };
  ...
  const filtered = rows.filter((r) => {
    const matchQ =
      !q ||
      r.ticket_no.toLowerCase().includes(q.toLowerCase()) ||
  ...
  const create = async (e: React.FormEvent) => {
    ...
    const ticket =
      form.ticket_no ||
      `SC-${new Date().getFullYear()}-${String(rows.length + 1).padStart(4, "0")}`;
    try {
      ...
      <Panel
        ...
        actions={
          <button
            ...
            className="flex items-center gap-1 rounded bg-primary px-2 py-1 text-[11px] font-medium text-primary-foreground hover:bg-primary/90"
          >
        ...
        <div className="mb-2 flex flex-wrap items-center gap-2">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search ticket/product/customer"
          ...
          <div className="flex items-center gap-1">
            ...
            <select
              ...
              className="rounded border border-input bg-background py-1 px-2 text-xs"
            >
                ...
                <tr key={r.id}>
                  ...
                  <td>
                    ...
                    <div className="text-[10px] text-muted-foreground">
                      {r.customers?.phone} · {r.customers?.city}
                    </div>
                  </td>
                  <td>
                    {r.product}
                    <div className="text-[10px] text-muted-foreground">{r.serial_no}</div>
                  </td>
                  ...
                  <td>
                    <select
                      ...
                      className="rounded border border-input bg-background py-0.5 px-1 text-[11px]"
                    >
                      <option value="open">open</option>
                      <option value="allocated">allocated</option>
                      <option value="in-progress">in-progress</option>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={8} className="py-6 text-center text-muted-foreground">
                    No service calls match filters.
                  </td>
                </tr>
              )}
            </tbody>
